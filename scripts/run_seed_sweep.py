#!/usr/bin/env python3
"""Run fixed-budget multi-seed experiments for Step 4."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run fixed-budget Step 4 seed sweeps.")
    ...
    parser.add_argument("--train_episodes", type=int, default=3000,
                        help="Training budget for Step 4.")
    parser.add_argument("--eval_freq", type=int, default=200,
                        help="Checkpoint every N episodes. 200 balances cost vs resolution "
                             "when evaluating the full training set (~262 starts).")
    parser.add_argument("--validation_days", type=int, default=0,
                        help="Val split size. 0 = no val split; checkpoint selection uses "
                             "all training starts (SEM≈0.62 vs SEM≈2.0 for 24-episode val).")
    ...
    parser.add_argument("--methods", nargs="+", choices=("step4",), default=("step4",))
    parser.add_argument("--seeds", nargs="+", type=int, default=(7, 13, 29, 41, 97))
